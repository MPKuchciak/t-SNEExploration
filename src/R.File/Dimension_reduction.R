...
#' 
#' t-SNE, developed by Laurens van der Maaten and Geoffrey Hinton, stands out from dimensionality reduction techniques through its great capability to maintain local data relationships and potential to discover data clusters in an easily understood manner.
#' By converting high-dimensional Euclidean distances between data points into conditional probabilities that represent similarities, t-SNE allows for the visualization of multi-dimensional data in two or three dimensions. This makes t-SNE particularly valuable for datasets like MNIST, where understanding the distribution and separation of digit representations can provide insights into the complexity of the data.
#' 
#' ___
#' 
#' ## Methodology
...
#' 
## ----setup of loading, include=FALSE--------------------------------------------------------------------------------------------------------
...
#' 
#' ![MNIST sample of images](../images/mnist_animation_enlarged.gif)
#' 
#' ___
#' 
#' 
...
#' 
## ----flattening dataset2, include=FALSE-----------------------------------------------------------------------------------------------------
...
# Apply t-SNE with a selected perplexity and maximum iterations
tsne_results_p100_i1000 <- Rtsne(train_images_flat, dims = 2, perplexity = 100, verbose = TRUE, max_iter = 1000)
dim(train_images)


#' 
## ----flattening dataset3, include=FALSE-----------------------------------------------------------------------------------------------------
...
# Apply t-SNE with a selected perplexity and maximum iterations
tsne_results_p5_i1000 <- Rtsne(train_images_flat, dims = 2, perplexity = 5, verbose = TRUE, max_iter = 1000)
dim(train_images)

#' 
## ----flattening dataset4, include=FALSE-----------------------------------------------------------------------------------------------------
...
# Apply t-SNE with a selected perplexity and maximum iterations
tsne_results_p30_i2000 <- Rtsne(train_images_flat, dims = 2, perplexity = 30, verbose = TRUE, max_iter = 2000)
dim(train_images)

#' 
## ----flattening dataset5, include=FALSE-----------------------------------------------------------------------------------------------------
...
# Apply t-SNE with a selected perplexity and maximum iterations
tsne_results_p30_i100 <- Rtsne(train_images_flat, dims = 2, perplexity = 5, verbose = TRUE, max_iter = 100)
dim(train_images)
...
#' 
#' **Perplexity**: Perplexity is a measure of the effective number of neighbors each point considers in its local neighborhood. It's one of the most influential hyperparameters in t-SNE and directly affects the balance between emphasizing local versus global structure in the data.
#' Perplexity impacts how t-SNE views and emphasizes the relationships between points. A low perplexity focuses on very local structure, potentially missing broader patterns. 
#' 
...
#' 
#' **Theta (Barnes-Hut t-SNE)**:Theta is specific to the Barnes-Hut approximation of t-SNE, a speedup that makes the algorithm feasible for larger datasets. It controls the trade-off between speed and accuracy in this approximation.
#' A lower theta value means the algorithm will use a more accurate but slower approximation to calculate the interactions between points. A higher theta speeds up the computation but at the cost of some accuracy.
#' 
...
#' 
#' In summary, tuning perplexity, the number of iterations, and theta in t-SNE is a balancing act between preserving local versus global data structures, achieving computational efficiency, and ensuring the stability and accuracy of the embeddings. 
#' Systematic experimentation, guided by visual and quantitative assessments, is the best approach to finding the optimal settings for your specific dataset and analysis goals.
#' 
...
#' 
#' *Distinct Clusters*: The visualization displays distinct clusters of data points, each representing one of the ten digit classes (0 through 9) in the MNIST dataset. 
#' These clusters are color-coded for clarity.
#' 
## ----all graphs1, echo=TRUE-----------------------------------------------------------------------------------------------------------------
...
#' 
## ----all graphs2, echo=FALSE----------------------------------------------------------------------------------------------------------------

library(ggplot2)

df_tsne <- data.frame(X = tsne_results_p100_i1000$Y[,1], Y = tsne_results_p100_i1000$Y[,2], Digit = as.factor(train_labels))
ggplot(df_tsne, aes(x = X, y = Y, color = Digit)) +
  geom_point(alpha = 0.7) +
  theme_minimal() +
  labs(title = "MNIST Digits Visualized via t-SNE p 100 i 1000", x = "t-SNE Dimension 1", y = "t-SNE Dimension 2", color = "Digit Label")
...
#' 
#' *Cluster Dispersio*n: The individual clusters appear more dispersed. For example, the clusters for digits '3', '5', and '8' are not as compact as seen with lower perplexity, which could indicate a loss of detail in the local structure of the data.
#' 
...
#' 
## ----all graphs3, echo=FALSE----------------------------------------------------------------------------------------------------------------

library(ggplot2)

df_tsne <- data.frame(X = tsne_results_p5_i1000$Y[,1], Y = tsne_results_p5_i1000$Y[,2], Digit = as.factor(train_labels))
ggplot(df_tsne, aes(x = X, y = Y, color = Digit)) +
  geom_point(alpha = 0.7) +
  theme_minimal() +
  labs(title = "MNIST Digits Visualized via t-SNE p 5 i 1000", x = "t-SNE Dimension 1", y = "t-SNE Dimension 2", color = "Digit Label")
...
#' 
## ----all graphs4, echo=FALSE----------------------------------------------------------------------------------------------------------------

library(ggplot2)

df_tsne <- data.frame(X = tsne_results_p30_i2000$Y[,1], Y = tsne_results_p30_i2000$Y[,2], Digit = as.factor(train_labels))
ggplot(df_tsne, aes(x = X, y = Y, color = Digit)) +
  geom_point(alpha = 0.7) +
  theme_minimal() +
  labs(title = "MNIST Digits Visualized via t-SNE p 30 i 2000", x = "t-SNE Dimension 1", y = "t-SNE Dimension 2", color = "Digit Label")
...
#' 
## ----all graphs5, echo=FALSE----------------------------------------------------------------------------------------------------------------

library(ggplot2)

df_tsne <- data.frame(X = tsne_results_p30_i100$Y[,1], Y = tsne_results_p30_i100$Y[,2], Digit = as.factor(train_labels))
ggplot(df_tsne, aes(x = X, y = Y, color = Digit)) +
  geom_point(alpha = 0.7) +
  theme_minimal() +
  labs(title = "MNIST Digits Visualized via t-SNE p 30 i 100", x = "t-SNE Dimension 1", y = "t-SNE Dimension 2", color = "Digit Label")
...
#' #### Summary
#' 
#' The baseline setting (perplexity 30, iterations 1000) is effective for MNIST, providing a good starting point for visualization.
#' Increasing perplexity can sometimes merge clusters that should be distinct, indicating that the chosen perplexity may be too high for preserving the local structure of MNIST.
#' 
...
## ----final_res, echo=FALSE------------------------------------------------------------------------------------------------------------------

# Ensure the Rtsne library is loaded
library(Rtsne)

# Assuming train_images_flat is your preprocessed data ready for t-SNE
tsne_results <- Rtsne(train_images_flat, dims = 2, perplexity = 30, verbose = FALSE, max_iter = 1000)

# Access the final value in itercosts, which should represent the final KL divergence
final_kl_divergence <- tail(tsne_results$itercosts, n = 1)
cat("Final KL Divergence:", final_kl_divergence, "\n")
...
#' The cost function minimized by t-SNE is non-convex, leading to potential variance in results across different runs, especially with different initializations or slight changes in hyperparameters. This can make the interpretation of results somewhat subjective and potentially inconsistent.
#' 
#' **Interpretation Challenges**
#' t-SNE is excellent at revealing local structures and clusters but does not preserve global distances well. This means that while clusters are meaningful, the distance between clusters might not be. It's a mistake to interpret the absolute or relative positioning of clusters in t-SNE plots as indicative of their real-world similarity or dissimilarity.
#' The algorithm tends to expand dense clusters and contract sparse ones, which can sometimes exaggerate separations between groups.
#' 
#' **Scalability and Computational Efficiency**
#' t-SNE can be computationally intensive, particularly on large datasets. The Barnes-Hut approximation improves efficiency by approximating long-range interactions and reducing complexity, but scalability remains a challenge for very large datasets.
#' 
#' **The Crowding Problem and Dimensionality Curse**
#' The "crowding problem" describes a situation where, in reducing dimensions, t-SNE tends to squeeze together points that are not necessarily close in the high-dimensional space. This is related to the curse of dimensionality, where distances in high-dimensional spaces become less meaningful, making it hard for any low-dimensional representation to perfectly capture high-dimensional distances.
#' 
#' **Difficulty in Reproducing Results**
#' Due to its stochastic nature and sensitivity to hyperparameters, reproducing t-SNE results can be difficult without careful documentation of parameter settings and random seeds.
#' 
#' ## Conclusion
#' 
