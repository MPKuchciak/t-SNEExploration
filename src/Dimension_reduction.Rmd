...
```{r setup of loading, include=FALSE}
...
- 
$$ p_{j|i} $$
: Conditional probability that point 
$$ x_i $$ 
would pick 
$$x_j$$ 
as its neighbor, based on their distance and the density of points around 
$$x_i$$.
- 
$$\sigma_i$$
: The standard deviation of the Gaussian distribution centered on 
$$x_i$$
, adjusted for each point to maintain a consistent perplexity across the dataset.

#### **Step 2**: Symmetrize the Probability Distribution

Define the joint probability 
$$p_{ij}$$ 
as the symmetrized conditional probabilities.
...
```{r flattening dataset1, echo=TRUE}
...
```{r flattening dataset2, include=FALSE}
...
```{r flattening dataset3, include=FALSE}
...
```{r flattening dataset4, include=FALSE}
...
```{r flattening dataset5, include=FALSE}
